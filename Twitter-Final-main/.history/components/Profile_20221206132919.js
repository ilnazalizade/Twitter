        ...
        <div
            ...
            <div className="flex py-2 px-3 sticky top-0 bg-white border-b border-gray-200">
                <div className="flex items-center">
                    ...
                    <h2 className="text-lg sm:text-xl font-bold cursor-pointer ml-4">{user?.name}</h2>
                </div>
            ...
            {user &&
            <>
                <header style={{backgroundImage:`url(${user?.headerImg})`}} className="h-[200px] h-full bg-[#B2B2B2] relative bg-cover">
                    <div className="w-[135px] h-[135px] bg-black rounded-full bg-white border-4 absolute top-32 left-4 border-3 border-white cursor-pointer overflow-hidden">
                        <img className="w-[100%]" src={user?.userImg ? user?.userImg : "/avatar.png"} alt="avatar"/>
                    </div>
                </header>
                        ...
                        <button onClick={follow} className="bg-black mt-4 mr-4 text-white px-4 pt-2 pb-2 mr-2 hover:bg-opacity-75 text-[15px] rounded-3xl font-bold">
                            {
                                handleFollow ?
                                    'Unfollow'
                                    :
                                    user?.followers?.indexOf(uid) === -1 ? 'Follow' : 'Unfollow'
                            }
                        </button>
                        :
                ...
                <div className="mt-6">
                    <h2 className="text-lg sm:text-xl font-bold cursor-pointer ml-4">{user?.name}</h2>
                    <p className="text-[15px] text-gray-500 ml-4">@{user?.username}</p>
                </div>
                {
                    user?.bio && <p className="text-[15px] ml-4 mt-[12px]">{user?.bio}</p>
                }
                ...
                <div className="flex items-center ml-4 mt-3">
...

                    {
                        ...
                        <>
                            ...
                            <p className="text-[15px] text-gray-500 ml-1 mr-3">{user?.location}</p>
                        </>
            ...
            <div>
                <AnimatePresence>
                    {tweets.map((item) => {
                        return (!user || item?.data()?.id === currentUser?.uid) &&
                            <motion.div
                                key={item.id}
                                initial={{ opacity: 0 }}
                                animate={{ opacity: 1 }}
